...
from dataloaders.la_heart_d import (LA_heart, CenterCrop, RandomCrop,
                                   RandomRotFlip, ToTensor,
                                   TwoStreamBatchSampler)
from networks.net_factory_3d import net_factory_3d
from utils import losses, metrics, ramps, VAT1
...
parser.add_argument('--root_path', type=str,
                    default='/home/xsq/xsq/TSS-CL/my_data/dwh/train_50_50', help='Name of Experiment')##### Change dataset
parser.add_argument('--val_path', type=str,
                    default='/home/xsq/xsq/TSS-CL/my_data/dwh/val', help='Name of Experiment')
...
parser.add_argument('--clean_labeled_num', type=int, default=100,##### Accordingly change HQ setting
                    help='labeled data')
parser.add_argument('--total_sample', type=int, default=200,
                    help='total samples')
# costs-
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            with torch.no_grad():            
                ema_output1_no_noise = ema_model1(LQ_labeled_volume_batch)
                ema_output1_soft_no_noise = torch.softmax(ema_output1_no_noise, dim=1)
                ema_output2_no_noise = ema_model2(LQ_labeled_volume_batch)
                ema_output2_soft_no_noise = torch.softmax(ema_output2_no_noise, dim=1)
                outputs1 = model(LQ_labeled_volume_batch)
            ...
            try:
                ...
                if correct_type == 'smooth':
                    ...
                    for i in range(T//2):
                        ...
                        with torch.no_grad():
                            preds0[2 * stride * i:2 * stride *(i + 1)] = model(ema_inputs)
                            preds1[2 * stride * i:2 * stride *(i + 1)] = ema_model1(ema_inputs)
                            preds2[2 * stride * i:2 * stride *(i + 1)] = ema_model2(ema_inputs)
            ...
            logging.info(
                'iteration %d : loss : %f, loss_ce: %f, loss_dice: %f, consistency_loss: %f, weak_supervised_loss: %f' %
                (iter_num, loss.item(), loss_ce.item(), loss_dice.item(), consistency_loss, weak_supervised_loss))
...
if __name__ == "__main__":
    ...
    snapshot_path = "../VAT_SErr/model_STT_50_50_7/{}_{}/{}".format(
